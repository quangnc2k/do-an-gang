...
type AlertConfigRepository interface {
	...
	UpdateOneID(ctx context.Context, config model.AlertConfig, id string) (err error)
	DeleteOneByID(ctx context.Context, id string) (config model.AlertConfig, err error)
}
